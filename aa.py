...
def add_task(list_category: list, list_task: list) -> list:
    while True:
        task = {'text': input("Introduceti taskul: "),
                ...
                'categorie': input(f"Introduceti o categorie din care face parte taskul ({', '.join(list_category)}): ").lower()}
        while task['categorie'] not in list_category:
            task['categorie'] = input(
                f"Categoria aleasa nu exista. Va rog introduceti o categorie din lista ({', '.join(list_category)}): ").lower()
        check = False
...
def editare(list_category: list, list_task: list) -> list:
    ...
    if choice_camp == '1':
        ...
        list_task[choice_task]['categorie'] = input(
            f"Introduceti o categorie din care face parte taskul ({', '.join(list_category)}): ").lower()
